...
void CreateProgramList(){
	...
	fptr_list = fopen("object_code.txt","w");

    if (fptr_list == NULL)
	{
		printf("ERROE: Unable to open the object_code.txt file\n");
		exit(1);
	}
...
void CreateObjectCode(){
	...
	FILE *fptr_obj;    
	fptr_obj = fopen("object_program.txt","w");
    if (fptr_obj == NULL)
	{
		printf("ERROE: Unable to open the object_progrm.txt\n");
		exit(1);
	}
...
int main ()
{
	...
	if (fptr == NULL)
	{
		printf("ERROE: Unable to open the input.txt file.\n");
		exit(1);
	}

//PASS 1
	printf("Pass 1 Running...\n");
    ...
    printf("Length of the program : %06x",program_length);
